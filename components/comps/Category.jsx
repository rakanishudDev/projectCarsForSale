import AutoMarkSelect from "../elements/AutoMarkSelect"
...
const Category = ({onSearch}) => {
    ...
    const changeSearchPath = (category, id) => {
        
        let path = searchPath;
      
        if (category === 'all') {
            console.log(searchData[id])
            path = path.filter(cat => searchData[id].toUpperCase() !== cat.name)
            setSearchPath(path)
        } else {
            
            path.map(cat => {
                if (cat.id === id) {
                    return path = path.filter(catX => catX.id !== id)
                }
            
            })
            if (id === 'offer') {
                if (!category) {
                    path = path.filter(cat => cat.id !== id)
                    setSearchPath(path)
                    return
                }
                console.log(category)
                path.push({name: 'OFFER', id})
                setSearchPath(path)
                return
            }
            let value = category.toUpperCase();
            path.push({name: value, id})
            setSearchPath(path)
        }
    ...
    const onSelectVehicle= (e) => {
        if (e.target.id === 'offer') {
            ...
            changeSearchPath(e.target.value, e.target.id)

        }
        if (e.target.id === 'vehicleType') {
            setSearchData(prevState => {
                return {...prevState, vehicleType: e.target.value, bodytype: 'all'}
            })
        }
        let makeV = null
        if (e.target.id === 'make') {
        ...
        setSearchData(prevState => {
            return {...prevState, [e.target.id]: boolean?? checked?? makeV?? e.target.value}
        })
    }
    ...
    <div className={styles.categoryContainer}>
        ...
        <div className={styles.selectContainer}>
            ...
            <select className={styles.categorySelect} defaultValue="hidden" id="bodytype" onChange={onSelectVehicle}>
                ...
                {vehicleType === 'all' && (<option disabled>Choose Vehicle Type First</option>)}
                <option value="all">all</option>
                </optgroup>
        ...
        <ul className={styles.searchPathContainer}>
            {searchPath.map(category => {
                return <li className={styles.searchPath} key={category.name + category.id}>{category.name}</li>
            }) }
        </ul>
    </div>
  )
